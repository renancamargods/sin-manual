...
export const tecnicoPages: TecnicoGroup[] = [
  {
    ...
    pages: [
      ...
      { title: 'Faturamento de Pedido e Movimento de Serial', slug: 'faturamento-e-movimento-serial', subgroup: 'Faturamento', order: 1, description: 'Fluxo técnico de HML/QA: do pedido aprovado ao faturamento no SAP, à validação da nota fiscal e à inserção do movimento serial usado em devoluções e trocas.' },
      { title: 'Troca, Devolução e Cancelamento (TDC)', slug: 'troca-devolucao-cancelamento-tdc', subgroup: 'Faturamento', order: 2, description: 'Fluxo do Pedido TDC: triagem que recebe itens de volta, gera pedidos de devolução, troca ou consignado e apura crédito ou débito para o cliente.' },
      { title: 'Fechamento de Consignado', slug: 'fechamento-de-consignado', subgroup: 'Faturamento', order: 3, description: 'Fluxo do fechamento de consignado: selecionar os itens que ficaram em consignação no cliente, equalizar desconto e gerar o pedido de faturamento — por cliente ou por nota fiscal.' },
      { title: 'Transferência Reversa', slug: 'transferencia-reversa', subgroup: 'Faturamento', order: 4, description: 'Fluxo da transferência reversa: bipar os itens que voltam de um Lounge para outro (Centro origem → destino), registrar o retorno no SAP e finalizar a transferência.' },
      { title: 'Integração com o SAP (S/4HANA) - Visão Geral', slug: 'integracao-sap-visao-geral', subgroup: 'Integração SAP', order: 1, description: 'Como funciona a integração com o SAP S/4HANA: motor de etapas dirigido por configuração, os quatro fluxos encadeados, estados e monitoramento.' },
      { title: 'Integração SAP - Os quatro fluxos', slug: 'integracao-sap-os-quatro-fluxos', subgroup: 'Integração SAP', order: 2, description: 'Passo a passo dos quatro fluxos de integração com o SAP: Business Partner (cliente), Sales Order (pedido), Outbound Delivery (remessa) e Billing Document (fatura).' },
      { title: 'Pagamentos - Geração de Link (PIX, Cartão e Maquininha)', slug: 'pagamentos-geracao-de-link', subgroup: 'Pagamentos', order: 1, description: 'Como o pedido gera cobrança após a aprovação: roteamento pós-aprovação, link PIX (EFI/GerenciaNet), link de cartão e cobrança na maquininha (Pagar.me), chave PIX e notificações.' },
      { title: 'Pagamento Misto', slug: 'pagamento-misto', subgroup: 'Pagamentos', order: 2, description: 'Fluxo do pagamento misto: combinar mais de uma forma de pagamento no mesmo pedido (boleto, PIX e cartão), com o ciclo de status de token, cobrança e confirmação.' },
      { title: 'Simular Pagamentos em Homologação (QA)', slug: 'simular-pagamentos-homologacao', subgroup: 'Pagamentos', order: 3, description: 'Procedimento de QA para confirmar pagamentos de cartão em Homologação (sem captura real): PagarMe (webhook charge.paid), PayGo (gerador de cartão VISA) e Pagamento Misto (mutation GraphQL).' },
      { title: 'Ocorrência PFCQ - do recebimento ao encerramento', slug: 'ocorrencia-pfcq', subgroup: 'Ocorrências', order: 1, description: 'Fluxo da Ocorrência PFCQ: registrar e analisar uma queixa técnica/não conformidade de produto, do recebimento à análise técnica, procedência, faturamento e encerramento.' },
      { title: 'Análise de Qualidade e Refugo', slug: 'analise-de-qualidade-e-refugo', subgroup: 'Qualidade', order: 1, description: 'Fluxo de análise de qualidade: triar e classificar os produtos que retornam (vendável ou sucata), transferir para o SAP/Delage e aplicar as regras de refugo por produto e lote.' },
      { title: 'Leads - do Lead à Pipeline (Kanban) e conversão', slug: 'leads-pipeline-e-conversao', subgroup: 'Leads', order: 1, description: 'Fluxo do lead: da entrada e qualificação à movimentação pela pipeline no Kanban, com histórico de interação e alertas de SLA, até o fechamento (ganho ou perda).' },
      { title: 'Crédito e Antifraude (ClearSale)', slug: 'credito-e-antifraude-clearsale', subgroup: 'Crédito', order: 1, description: 'Análise de crédito e antifraude no cadastro do cliente com a ClearSale: score de fraude, insights, ratings, antecedentes, biometria facial e classificação de crédito.' },
      { title: 'Motor de Crédito (Análise Automática do Pedido)', slug: 'motor-de-credito', subgroup: 'Crédito', order: 2, description: 'Como o Motor de Crédito (regras R006) analisa o pedido no envio: sequência de regras que decide entre aprovar automaticamente ou mandar para a Mesa de Crédito, e onde a decisão aparece.' },
      { title: 'Verificar a Decisão do Motor de Crédito (QA)', slug: 'verificar-decisao-motor-de-credito', subgroup: 'Crédito', order: 3, description: 'Procedimento de QA/suporte para conferir o que o Motor de Crédito decidiu em um pedido: pontos de verificação e como ler o log por regra.' },
      { title: 'Motor de Crédito - Análise Automática do Pedido', slug: 'motor-de-credito-analise-automatica', subgroup: 'Crédito', order: 2, description: 'Análise automática do pedido antes das filas: a árvore de decisão do Motor de Crédito (Blacklist, Depósito, Cartão/PIX, Bonificação, OPL, Pagamento Misto, Serasa, CAR e Valor) e as 12 validações da alçada Adm. Vendas que geram os motivos de bloqueio.' },
      { title: 'GO.ON - Agenda e Roteirização de Campo', slug: 'goon-agenda-e-roteirizacao', subgroup: 'Roteirizador', order: 1, description: 'Fluxo do atendimento de campo: montar a agenda, associar o formulário, definir disponibilidade (jornada/ausência) e integrar ao GO.ON, que gera a OS e executa no mobile.' },
      { title: 'Contrato - do rascunho às assinaturas e comodato', slug: 'contrato-cadastro-aprovacao-assinatura', subgroup: 'Contratos', order: 1, description: 'Fluxo do contrato: do rascunho à aprovação pelos grupos responsáveis, à assinatura eletrônica (ZapSign) e à geração dos pedidos de comodato, com renovação, aditivo e cancelamento.' },
      { title: 'Solicitações HandsOn (Logística de Treinamento)', slug: 'solicitacoes-handson-treinamento', subgroup: 'Ocorrências', order: 2, description: 'Fluxo das Solicitações HandsOn: abrir e conduzir pedidos de logística de treinamento (professor, material ou visita à fábrica) vinculados aos cursos.' },
      { title: 'Planejamento de Vendas e Cotas', slug: 'planejamento-de-vendas-e-cotas', subgroup: 'Planejamento de Vendas', order: 1, description: 'Fluxo do planejamento de vendas: distribuir e ajustar as metas/cotas de venda pela estrutura comercial, do topo ao consultor, com registro de quem alterou.' },
  ...
  { title: 'Mapa de Fluxos', slug: 'mapa-de-fluxos', description: 'Visão de conjunto dos fluxos e como eles se conectam, do lead ao faturamento e ao pós-venda.', pages: [] },
  ...
  { title: 'Templates', slug: 'templates', description: 'Modelos reutilizados em documentos, mensagens e etiquetas.', pages: [] },
  {
    ...
    slug: 'regras-de-validacao',
    description: 'Regras de validação de campo das telas (o que trava o Salvar/Enviar), por tela.',
    pages: [
      { title: 'Cliente', slug: 'cliente', order: 1, description: 'Regras de validação do cadastro de Cliente (obrigatórios por tipo de pessoa, documento, fiscais, CRO, contatos).' },
      { title: 'Pedido', slug: 'pedido', order: 2, description: 'Regras de validação do Pedido (obrigatórios, estrutura comercial, itens, pagamento, limites).' },
      { title: 'Lead', slug: 'lead', order: 3, description: 'Regras de validação do cadastro de Lead (obrigatórios, documento/e-mail, cliente Base SIN, valor potencial).' },
      { title: 'Contrato', slug: 'contrato', order: 4, description: 'Regras de validação do Contrato (obrigatórios, estrutura comercial, vigência, rebate, Curso e ZapSign).' },
      { title: 'Fechamento de Consignado', slug: 'fechamento-de-consignado', order: 5, description: 'Regras de validação do Fechamento de Consignado (obrigatórios da solicitação e regras de desconto).' },
      { title: 'Produto', slug: 'produto', order: 6, description: 'Regras de validação do cadastro de Produto (obrigatórios, dimensões/pesos e limites de tamanho).' },
      { title: 'Orçamento', slug: 'orcamento', order: 7, description: 'Regras de validação do Orçamento (obrigatórios, limites e regras dos itens).' },
      { title: 'Trocas/Devoluções (TDC)', slug: 'trocas-devolucoes-tdc', order: 8, description: 'Regras de validação do Pedido TDC (triagem e item de troca).' },
      { title: 'Política Comercial', slug: 'politica-comercial', order: 9, description: 'Regras de validação da Política Comercial (obrigatórios, vigência, desconto, range e condicionais por Tipo/Regra).' },
      { title: 'Condição de Pagamento', slug: 'condicao-de-pagamento', order: 10, description: 'Regras de validação da Condição de Pagamento (obrigatórios e faixas de prazo/parcelas/código ERP).' },
      { title: 'Ocorrência', slug: 'ocorrencia', order: 11, description: 'Regras de validação da Ocorrência (Observações, Cliente, Centro e Consultor FVE obrigatórios).' },
...
export const modules: ModuleDef[] = [
  {
    ...
    routines: [
      ...
      { title: 'Serviço Agendado', slug: 'servico-agendado', description: 'Cadastro de serviços agendados.', unavailable: true },
      { title: 'Execução de Serviço Agendado', slug: 'execucao-de-servico-agendado', description: 'Acompanhamento das execuções de serviços agendados.', unavailable: true },
  ...
  },
  {
    ...
    routines: [
      ...
      { title: 'Caixas do Pedido', slug: 'caixas-do-pedido', description: 'Controle de caixas usadas no envio do pedido.', unavailable: true },
      { title: 'Categoria Item', slug: 'categoria-item', description: 'Categoria usada para classificar itens do pedido.' },
      { title: 'Nova Fila de Aprovação - Pedidos', slug: 'nova-fila-de-aprovacao-pedidos', description: 'Fila de pedidos aguardando aprovação.', unavailable: true },
      ...
      { title: 'Log de Faturamento SAP', slug: 'log-de-faturamento-sap', description: 'Acompanhamento do envio de faturamento ao SAP.', unavailable: true },
      ...
      { title: 'Pedido para Integração', slug: 'pedido-para-integracao', description: 'Pedidos preparados para integração com outros sistemas.', unavailable: true },
...
export const tecnicoGroupEn: Record<string, { title: string; description: string }> = {
  ...
  'mapa-de-fluxos': { title: 'Flow Map', description: 'A bird\'s-eye view of the flows and how they connect, from lead to invoicing and after-sales.' },
  ...
  'regras-de-validacao': { title: 'Validation Rules', description: 'Field validation rules of the screens (what blocks Save/Submit), per screen.' },
};
...
export const tecnicoSubEn: Record<string, string> = {
  ...
  'fluxos/faturamento-e-movimento-serial': 'Order Invoicing and Serial Movement',
  'fluxos/troca-devolucao-cancelamento-tdc': 'Exchange, Return and Cancellation (TDC)',
  'fluxos/fechamento-de-consignado': 'Consignment Closing',
  'fluxos/transferencia-reversa': 'Reverse Transfer',
  'fluxos/integracao-sap-visao-geral': 'SAP (S/4HANA) Integration - Overview',
  'fluxos/integracao-sap-os-quatro-fluxos': 'SAP Integration - The four flows',
  'fluxos/pagamentos-geracao-de-link': 'Payments - Link Generation (PIX, Card and Card Terminal)',
  'fluxos/pagamento-misto': 'Mixed Payment',
  'fluxos/simular-pagamentos-homologacao': 'Simulate Payments in Homologation (QA)',
  'fluxos/ocorrencia-pfcq': 'PFCQ Occurrence - from receipt to closing',
  'fluxos/analise-de-qualidade-e-refugo': 'Quality Analysis and Scrap',
  'fluxos/leads-pipeline-e-conversao': 'Leads - from Lead to Pipeline (Kanban) and conversion',
  'fluxos/credito-e-antifraude-clearsale': 'Credit and Anti-fraud (ClearSale)',
  'fluxos/motor-de-credito': 'Credit Engine (Automatic Order Analysis)',
  'fluxos/verificar-decisao-motor-de-credito': 'Verifying the Credit Engine Decision (QA)',
  'fluxos/motor-de-credito-analise-automatica': 'Credit Engine - Automatic Order Analysis',
  'fluxos/goon-agenda-e-roteirizacao': 'GO.ON - Field Scheduling and Routing',
  'fluxos/contrato-cadastro-aprovacao-assinatura': 'Contract - from draft to signatures and lending',
  'fluxos/solicitacoes-handson-treinamento': 'HandsOn Requests (Training Logistics)',
  'fluxos/planejamento-de-vendas-e-cotas': 'Sales Planning and Quotas',
  ...
  'regras-de-validacao/cliente': 'Validation Rules - Customer',
  'regras-de-validacao/pedido': 'Validation Rules - Order',
  'regras-de-validacao/lead': 'Validation Rules - Lead',
  'regras-de-validacao/contrato': 'Validation Rules - Contract',
  'regras-de-validacao/fechamento-de-consignado': 'Validation Rules - Consignment Closing',
  'regras-de-validacao/produto': 'Validation Rules - Product',
  'regras-de-validacao/orcamento': 'Validation Rules - Quote',
  'regras-de-validacao/trocas-devolucoes-tdc': 'Validation Rules - Exchanges/Returns (TDC)',
  'regras-de-validacao/politica-comercial': 'Validation Rules - Commercial Policy',
  'regras-de-validacao/condicao-de-pagamento': 'Validation Rules - Payment Term',
  'regras-de-validacao/ocorrencia': 'Validation Rules - Occurrence',
...
export const tecnicoSubgroupEn: Record<string, string> = {
  ...
  'Integração SAP': 'SAP Integration',
  Pagamentos: 'Payments',
  ...
  Roteirizador: 'Router',
  Contratos: 'Contracts',
  'Planejamento de Vendas': 'Sales Planning',
};
